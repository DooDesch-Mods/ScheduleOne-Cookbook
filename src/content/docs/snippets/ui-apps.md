---
title: UI & Phone Apps
description: A full basic phone-app UI builder and the in-game phone canvas dimensions.
sidebar:
  order: 4
---

Building a custom phone App in Schedule I means constructing a uGUI hierarchy - top bar, scrolling list,
detail panel, buttons - by hand. This page has a complete, reusable example of that, plus the exact canvas
dimensions the phone uses so your layout lines up.

For building UI from a stripped Unity project and the general Unity GUI wrapper, see
[Unity & AssetBundles](/unity-assetbundles/). This page is just drop-in C#.

## Phone canvas dimensions

The phone screen is **1200 x 655** in horizontal orientation (swap for vertical). A handy authoring trick:
set your `Canvas` render mode to **World Space** first so you can size it to those exact dimensions and see
it, box out your panels, then switch back to **Overlay**. Keep every `RectTransform` anchored
center/middle - other anchors caused alignment issues. Many authors build the layout in Unity with the
same controller they then use in game, so they can iterate without launching the game repeatedly.

> Source: **Dreous** - [original message](https://discord.com/channels/1349221936470687764/1354830847659737122/1379192853233930240)

## Basic App UI builder

`AppUI` builds a two-panel app: a scrollable list on the left, a detail/overview panel with labelled value
rows and a call-to-action button on the right, plus a top bar with a live clock driven by the game's
`TimeManager`. Call `InitializeUI(container, onEntrySelected, onCallVehicle)` with your app's root
`RectTransform`, then feed it data with `ClearListItems()` and `AddListItem(...)`.

It expects an `EntryData` type (your own model) and uses `Action` callbacks, so wire those to your app
logic. It uses TextMeshPro (`TMPro`) for all text.

:::caution
This uses the Mono namespaces (`ScheduleOne.GameTime`, `ScheduleOne.DevUtilities`) and was, in the author's
words, put together with an LLM - treat it as a starting point and prune the verbose logging. On IL2CPP use
the `Il2CppScheduleOne.*` equivalents and interop-friendly delegate handling.
:::
...
namespace CallVehicle.Phone
{
    public class AppUI
    {
        ...
        private void CreateRightPanel(Transform parent)
        {
            ...
            initialText.fontStyle = FontStyles.Italic;
